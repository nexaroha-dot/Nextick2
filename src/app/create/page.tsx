...
import { Plus, FileText, MoreVertical, FileSignature, Edit, Trash, Copy, Eye, Share2 } from 'lucide-react';
import Link from 'next/link';
import ShareAccessModal from '@/components/ui/ShareAccessModal';
...
export default function CreateHubPage() {
  ...
  const loadTemplates = async () => {
    setIsLoading(true);
    // Fetch both forms and ticksheets and merge them (for now just ticksheets since builder is ticksheet only)
    const { getTemplates } = await import('@/actions/builder');
    const res = await getTemplates('ticksheet');
    if (res.success) {
  ...
  const handleCreateFormClick = () => {
    alert('Forms module is upcoming. Please create a Ticksheet instead.');
    setDropdownOpen(false);
  };

  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this questionnaire?')) return;
    const { deleteTemplate } = await import('@/actions/builder');
    const res = await deleteTemplate('ticksheet', id);
    if (res.success) {
  ...
  const handleCopy = async (id: string) => {
    const { duplicateTemplate } = await import('@/actions/builder');
    const res = await duplicateTemplate('ticksheet', id);
    if (res.success) {
      loadTemplates();
    } else {
    ...
    <div className="flex-1 flex flex-col h-full overflow-y-auto bg-transparent p-4 md:p-8 font-sans">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {questionnaires.map((q: any) => (
              <div key={q.id} className="glass-panel border border-slate-200/50 dark:border-slate-700/50 rounded-2xl p-4 md:p-5 hover:border-blue-500/50 hover:shadow-xl transition-all duration-300 group flex flex-col relative cursor-pointer overflow-hidden">
                    ...
                    {openMenuId === q.id && (
                      <>
                        ...
                        <div className="absolute right-0 mt-1 w-36 glass-panel border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-50 overflow-hidden py-1">
                          <Link href={`/builder?id=${q.id}`} className="w-full flex items-center gap-2 px-3 py-2 text-[12px] text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-left font-medium">
                            <Edit className="w-3.5 h-3.5 text-slate-400" /> Edit
                          </Link>
                          
                          <button onClick={(e) => { e.stopPropagation(); handleShareClick(q); }} className="w-full flex items-center gap-2 px-3 py-2 text-[12px] text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-left font-medium">
                            <Share2 className="w-3.5 h-3.5 text-blue-500" /> Share Access
                          </button>

                          <button onClick={(e) => { e.stopPropagation(); handleCopy(q.id); }} className="w-full flex items-center gap-2 px-3 py-2 text-[12px] text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-left font-medium">
                            <Copy className="w-3.5 h-3.5 text-slate-400" /> Duplicate
                          </button>
                          
                          <div className="border-t border-slate-100 dark:border-slate-800 my-1"></div>
                          
                          <button onClick={(e) => { e.stopPropagation(); handleDelete(q.id); }} className="w-full flex items-center gap-2 px-3 py-2 text-[12px] text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors text-left font-bold">
                ...
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-100 dark:border-slate-800/50">
                  <span className={`px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-widest ${q.status === 'Published' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'}`}>
                    {q.status}
                  </span>
      ...
      {shareModalOpen && shareTemplate && (
        <ShareAccessModal 
          ...
          templateType="ticksheet"
          initialAccessType={shareTemplate.access_type}
          initialAccessList={shareTemplate.access_list}
